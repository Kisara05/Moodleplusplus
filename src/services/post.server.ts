...
interface CreatePostDTO {
  ...
  sectionId: string;
  post_type: string; // 'material' | 'discussion'
}
...
export async function createPostWithContent({ title, content, sectionId, post_type }: CreatePostDTO) {
  if (!title || !content || !sectionId || !post_type) {
    return { error: "Title, Content, Type and Section ID are required" };
  }
  if (post_type !== 'post' && post_type !== 'discussion') {
  ...
  if (post_type === 'discussion' || post_type === 'post') {
